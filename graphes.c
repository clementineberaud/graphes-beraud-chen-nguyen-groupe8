...
void grapheMarkov(t_list_adj *list) {
	int verif = 1;
	// vérif : indique si c'est un graphe de Markov; il passe à 0 quand les probas d'un sommet ne sont pas égales à 1
	float somme = 0;
	for (int i = 0; i < list->taille; i++) {
		somme = 0;
		t_cell *temp = list->T[i].head;
		while (temp->next != NULL) {
			// somme des probas d'un sommet
			somme += temp->proba;
			temp = temp->next;
		}
		somme += temp->proba;
		if ((somme < 0.99) || (somme > 1)) {
			// on vérifie que la somme est entre 0.99 et 1
...
void carac_graphe_markov(t_list_adj graph) {
	...
	if (part.nb_composant==1) { // si 1 classe alors irréductible
		printf("Le graphe est irreductible.\n");
	} else {
		for (int j=0;j<tab->log_size;j++) { // sinon, on teste si des sommets de la classe ont un lien
			int classe=classes[tab->links[j].from-1];
			etats_classes[classe]=1; // on passe 1 s'il y a un lien
		}
	}
	...
	for (int k = 0; k < part.nb_composant; k++) {
		if (etats_classes[k]==1) { // si la classe = 1 alors transitoire
			...
			if (part.nb_sommet[k]==1) // on vérifie le nombre de sommets dans la classe
