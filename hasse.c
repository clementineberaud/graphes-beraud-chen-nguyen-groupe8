...
int link_in_tab (int a, int b, t_link_array *tab) { //on verifie si le lien est déjà dans le tableau
...
int* tab_classes (t_partition par, t_list_adj graph) { //tableau de classe par sommet
  ...
  for (int i=0;i<par.nb_composant;i++) { //on associe à l'indice i(=sommet i+1) le numéro de sa classe
...
t_link_array* diagramme_hasse (t_list_adj graph, t_partition par) { //renvoie un tableau de liens des classes
  ...
  for (int i=0;i<graph.taille;i++) {
    ...
    while (temp!=NULL) { //on regarde tous ses sommets d'arrivee
      int j=temp->sommet;

      int Cj=classes[j-1];
      if (Ci!=Cj) { //on vérifie que le sommet de départ et d'arrivée ne soient pas dans la meme classe
        int test=link_in_tab(i+1,j,tab); //test pour voir si leur lien est deja dans le tableau
      ...
      if (i+1==j && cpt_sommet(graph,i)==1){
        addLink(tab,i+1,j); //si le sommet n'a que lui meme en sommet d'arrivee on rajoute son lien
      }
